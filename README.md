...
GitHub API 客户端插件，按 `examples/demo.lazydeck` 的模块拆分方式组织：
...
- `/github/starred`
- `/github/gists`
- `/github/gists/<gist_id>`
- `/github/gists/<gist_id>/<filename>`
- `/github/search`
- `/github/search/repo`
...
- repo entry 上按 `S` 会 star 仓库；只有在 `/github/starred` 页面按 `S` 才会 unstar 仓库
- `/github/gists` 会展示当前 token 用户的 gists；进入 gist 后列出文件，文件预览会优先使用 API 返回的 `content`，否则读取 `raw_url`
- 在 gist 文件上按 `Enter` 会用 `$VISUAL`/`$EDITOR` 打开临时文件；退出编辑器后如果内容变化，会通过确认对话框询问是否 PATCH 推送到 GitHub
...
插件会通过 `deck.cache` 缓存 GitHub API 响应，并按场景使用不同 TTL：

- `notifications`：60 秒
- `gists`：60 秒，gist 详情和 raw 文件内容：5 分钟
...
插件仍然保留进程内 session 缓存；`deck.cache` 用于跨重载和重启复用结果。
