...
- `/github/repo/<username>/<repo>` 当前提供 `readme`、`issues`、`pulls`、`branches`、`tags` 条目
- `/github/repo/<username>/<repo>/branches` 会展示该仓库的分支；进入分支后通过 `file` 插件的 browser/provider 复用代码浏览、目录预览、文件图标和列表 display
- `/github/repo/<username>/<repo>/tags` 会展示该仓库的标签；进入标签后通过 `file` 插件的 browser/provider 复用代码浏览、目录预览、文件图标和列表 display
- GitHub 仓库代码浏览是只读的，不提供 `file` 插件里的新建、删除、重命名等本地文件操作
- 代码预览仍受 `code_max_lines` 和 `code_max_chars` 限制
- `/github/repo/<username>/<repo>/issues` 会展示该仓库的 open issues
- `/github/repo/<username>/<repo>/issues` 页面按 `s` 会搜索当前仓库 issues，结果路由为 `/github/repo/<username>/<repo>/issues/search/<query>`
- `/github/repo/<username>/<repo>/pulls` 会展示该仓库的 open pull requests
